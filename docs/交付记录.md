...
| 2 | `webpage_snapshot` 支持 `max_lines`（≤5000，字符预算按行数同步放大）；截断时如实报「截到第几行 + 少给多少元素」 | `snapshot.resolveSnapshotLimits`、`formatSnapshotOutput` |
| 3 | `webpage_locate` **默认不滚视口**（`scroll` 默认 false），并回 `in_viewport` —— 于是 locate 能用来验证 scroll 是否生效 | `provider.locate`、`tool-browser` |
| 4 | `webpage_scroll` 的 `ref` 变可选：不给就落在视口中心（整页滚动），不再要求先 snapshot | `provider.scroll` → `viewportCenter` |
...
| 工具 | 能力级 | 一句话 |
|---|---|---|
| `webpage_console` | read | 读会话的 console 环形缓冲（Runtime + Log 两域合流，按 message 高水位去重，最新在前；**默认只给当前文档**） |
| `webpage_network` | read | 列网络请求 / 按 requestId 取响应体（Network 事件从不重放，断开窗口期的请求按「会丢」处理；**默认只给当前文档**） |
| `webpage_execute` | **mutate** | 白名单制的高危逃生舱：一次发一条允许列表内的 CDP 命令（`Runtime.evaluate` 会执行任意表达式，Promise 会被 await） |
| `webpage_find` | read | 在最近一次 snapshot 的大纲上做零状态文本检索，不发任何 CDP 命令 |
| `webpage_locate` | read | 按 ref 现算视口坐标盒（backendNodeId 路线）+ `in_viewport`；**默认不动视口**，可选 `Overlay.highlightNode` 高亮 / `scroll=true` 居中 |
...
- **`webpage_tabs`**：`list` / `activate` / `close` 三个 action。只列本会话自己开的标签页
  （用户的标签页永不出现、永不关闭）；activate 依赖传输层支持，不支持时报
  `BROWSER_NOT_IMPLEMENTED` 而不是静默失败。
- **五个按 ref 的操作工具**：`webpage_click` / `webpage_fill` / `webpage_press` /
  `webpage_scroll` / `webpage_wait`。全部走 `refs.resolve()` 的**写前检查**：纪元失效
...
- **fake-llm 全链路验证**：脚本第 3 轮同轮发 `webpage_tabs(list)` + `webpage_click`
...
1. `webpage_open` → session_id（ref epoch 0）
2. `webpage_snapshot` → epoch 1，`[ref=e1]`
3. `webpage_tabs list` → 只列本会话控制的 1 个标签页
4. `webpage_click(ref=e1)` → 真实鼠标事件执行，导航被检测到，epoch 推进到 2，
   结果里正确广播「旧 ref 全部失效」
