...
**一片空白**（标题栏、菜单栏都在，内容区纯色），而 `browser_snapshot` / `browser_screenshot`
...
1. `activate` —— 模型调 `browser_tabs(action=activate)` 切标签时跑 `layout()`。
...
16:37:40  模型第一次 browser_tabs(list)，发现两个标签 → 发 activate → 撞上最小化 → 空白
```
